...
unless(node['s3fs-fuse'][:bluepill])
  mounted_directories.each do |dir_info|
    mount dir_info[:path] do
      ...
      dump 0
      pass 0
      options (["allow_other,url=https://s3.amazonaws.com,passwd_file=/etc/passwd-s3fs,use_cache=#{dir_info[:tmp_store] || '/tmp/s3_cache'},retries=20#{",noupload" if dir_info[:no_upload]},#{dir_info[:read_only] ? 'ro' : 'rw'}", dir_info[:options]].select {|x| not (x.nil? or x.empty?)}).join(',')
      action [:mount, :enable]
